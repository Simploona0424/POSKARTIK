...
function EditLocation() {
  ...
  const editLocation = location.state?.data
  const allStates = [
    "Andhra Pradesh",
    "Arunachal Pradesh",
    "Assam",
  ...
  useEffect(() => {
    if (editLocation) {
      setPantryName(editLocation.pantryName);
      setConcernPerson(editLocation.concernPerson);
      setContact(editLocation.contact);
      setAddress1(editLocation.address1);
      setAddress2(editLocation.address2);
      setState(editLocation.state);
      setCity(editLocation.city);
      setPincode(editLocation.pincode);
      setMap(editLocation.map);
      setLatitude(editLocation.latitude);
      setLongitude(editLocation.longitude);
      setTable(editLocation.table);
    }
  }, [editLocation]);
    ...
    <div className="flex h-screen">
      ...
      <main className="flex-1 w-full min-w-0 flex flex-col bg-[#F3F7F9]">
        ...
        <section className="m-5 flex-1 bg-white border-2 border-gray-200 rounded-b-xl p-8 mt-0">
          ...
          <h2 className="text-base max-xl:text-sm font-medium">
            Pantry Location / <span className="text-[#0A6637]">Edit</span>
          </h2>
          <form
